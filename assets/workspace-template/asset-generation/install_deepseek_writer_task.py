#!/usr/bin/env python3
"""Install an on-demand Windows task for DeepSeek writing.

The task runs through an ASCII path under the user's Codex home, then changes
into the workspace and calls ``python run_xhs_delivery.py --run-deepseek-writers``.
"""
...
RESULT_PATH = OUT / "deepseek-writer-task-result.json"
TASK_NAME = "XHS-DeepSeek-Writers"
...
def runner_dir() -> Path:
    return codex_home() / "xhs-deepseek-writers"
...
def write_runners() -> tuple[Path, Path]:
    ...
    ps1_path = directory / "run_deepseek_writers.ps1"
    cmd_path = directory / "run_deepseek_writers.cmd"
    log_path = directory / "deepseek-writers.log"
    ps1_path.write_text(
        "\n".join(
            [
                ...
                '"[$(Get-Date -Format o)] deepseek writers started" | Out-File -FilePath $Log -Append -Encoding utf8',
                "Set-Location -LiteralPath $Workspace",
                "& $Python '.\\run_xhs_delivery.py' '--run-deepseek-writers' 2>&1 | Out-File -FilePath $Log -Append -Encoding utf8",
    ...
    cmd_path.write_text(
        '@echo off\r\npowershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_deepseek_writers.ps1"\r\n',
        encoding="ascii",
    )
    ...
    completed = run_command(
        [
            ...
            (
                ...
                f"Register-ScheduledTask -TaskName {ps_quote(TASK_NAME)} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Install or manage the XHS DeepSeek writer task")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--install", action="store_true", help="Install the on-demand DeepSeek writer task")
    mode.add_argument("--uninstall", action="store_true", help="Remove the DeepSeek writer task")
    mode.add_argument("--status", action="store_true", help="Query the DeepSeek writer task")
    mode.add_argument("--run-now", action="store_true", help="Trigger the DeepSeek writer task immediately")
    return parser
